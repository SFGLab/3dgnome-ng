...
| 41 | EBV load as a trans covariate | The HGSVC RNA-seq on the workstation aligned to the EBV genome, NC_007605, total and BHRF1 per person, CPM. Test whether the person specific deviation tracks it, and put it in idea 29's model. A part of the deviation that is trans can never be carried by loops, compartments or models, so this sets the cis ceiling below +0.10 if it holds. On HPRC he finds 931 genes over 0.5 on 206 people. | half a day on the workstation | running, 2026-09-22 23:10. `ebv_genes.py` cuts the 86 genes of NC_007605.1 from its GenBank record, `ebv_quant.sh` builds one salmon index of GENCODE v40 plus those and requantifies the nine, `/mnt/storagelinux/_hgsvc/salmon/ebv/`, so a read chooses between human and viral transcripts; HG00512 carries 152,726 EBV reads at a 90.6 percent mapping rate. `ebv_person.py` then reads the load as counts per million mapped, in total and per viral gene, and tests it on the gene wise statistic, the share of the deviation's variance, an out of fold prediction per person and against the loop, distance and compartment deviations. |
| 42 | Allelic expression, the trans free target, idea 7's missing input | The colleague's formalism: within a person the ratio of the two haplotypes' expression cancels the trans term, `y = log(E_h1 / E_h2)`, regressed on haplotype differences `X = x_h1 - x_h2`. The RNA-seq and the 1000G phased chr1 panel for the nine are on the workstation; the allele counts come from a diploid transcriptome per person with bcftools consensus and salmon, or from an aligner and counts at heterozygous sites. The 3D side of this stays thin, phased loops are 2 to 4 percent and the chr1 SVs few, so the ASE table is first a second view of the deviation with the trans part removed, and only then idea 7's target. | a day | running, 2026-09-22 23:30, chained behind 41. `panel_exonic_snps.py` makes one pass over the 1000G high coverage phased chr1 panel for the nine's exonic heterozygous SNVs and the GTEx lead eQTL variants with the phased alleles; `diploid_transcripts.py` rebuilds every chr1 transcript from the reference exons and writes a person's heterozygous ones twice with the phased alleles substituted, `<tx>_h1` and `<tx>_h2`, beside the other chromosomes' GENCODE transcripts; `ase_quant.sh` indexes and quantifies each person, `/mnt/storagelinux/_hgsvc/salmon/ase/`; `ase_person.py` reads `y = log2(E_h1 / E_h2)` per gene at 20 or more allelic reads and tests the genotype in its allelic form, `slope * (alt on h1 - alt on h2)` at heterozygous lead eQTLs, and the allelic imbalance against the total deviation. Indels are left out, so a transcript's haplotypes differ at SNVs only. |
