...
| 48 | Pol II occupancy as the target | Idea 38's occupancy over the gene body, TSS plus 300 bp to the gene end per kb, in place of the RNA-seq as what the structure is read against. Polymerase loading is closer to the promoter event the loops describe than the RNA that survives processing and decay, so if the nearest element distance tracks occupancy better than expression the models describe loading and the rest of the gap is downstream of structure. Same features, same three statistics, occupancy beside expression. Caveat in the row: the occupancy and the loops are one library, and the promoter window's occupancy is the loop ends at the promoter, so only the body is a separate quantity. | a script on the workstation; our cluster end coverage now, his BAM coverage when it comes | open |
| 49 | Pol II coverage as the activity of elements | Idea 26 with coverage in place of peak signal: each of a person's own elements gets its Pol II coverage per kb from the library, a graded activity rather than a call, and the gene's feature is the distance to the nearest element above an activity quantile, and the ABC form summed over elements near in 3D with coverage as the activity. The untried variant of 26, whose linear form carried +0.08 of the person specific part and whose 3D form added nothing. | a script on the workstation on the chr1 models; element coverage from the laptop's cluster files | open |
| 50 | Transcription factories with loop dropout, 35 with 33 | The engine idea the user was first wary of, taken together with the many body term: each conformation draws each loop with a probability from its strength (33), and Pol II occupied beads attract as a group with occupancy setting how strongly each joins (35), so the ensemble carries both the frequency of a loop and the crowding of active promoters, and a person's count of a shared loop and their occupancy at an element both reach the mean structure. Input is promoter and element coverage only, each tested gene's body held out. Decided by 48 and 49 first, then engine work. | engine work in the arcs stage and a new term; needs 48 and 49 first and the 1D coverage as an input | open, after 48 and 49 |
...
- 2026-09-23. Ideas 48 to 50 added from the Pol II result: occupancy as the target, coverage as element activity, and factories with loop dropout; 48 and 49 running first.
- 2026-09-23, 02:20. Idea 38, his reply confirms the pipeline; with gene length held fixed Pol II occupancy sits at about 0.6 by every form, his 0.70 plain reads 0.65, the raw 0.42 reads 0.57; the earlier 0.45 reading is corrected.
