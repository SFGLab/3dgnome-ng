...
def mc_ib_numba(
    ...
    `arc_dist` is an (n, n) matrix of target distances between block centroids, built from the
    arcs whose anchors fall in different blocks. It rides the kernel's existing pairwise
    distance term, whose skip rule is a target below 1e-6, so unconnected block pairs cost
    nothing and no new energy code is needed.  All IBs move
