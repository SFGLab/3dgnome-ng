...
| `solver_tol` | float | 0.0 | 0.0 | The solve's stop rule. Positive stops it when an iteration improves the energy by less than this fraction of it, and `solver_iters` is then a safety cap; zero leaves the cap as the only stop. Under measurement. |
