...
| `solver_device` | str | cpu | cpu | Where the solver evaluates its energy. `cpu` is the numba kernel over every pair. `gpu` evaluates the same energy on the JAX device in float64, row chunked, so a chromosome solve costs one read of the target matrix per iteration instead of the CPU's visit of every pair; the sum order differs, so results agree to float precision and not to the bit. Needs JAX with a device. |
