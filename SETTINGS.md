...
Every key a config can carry, what it does, and what it is set to. Two values are given where
they differ. The default is what a run uses when the key is absent. The production value is
what the ensemble configs are generated with, and it is the one to compare against when reading
a result.
...
Some keys are accepted and never used. They are the reference implementation's own, kept so a
config written for it does not warn, and each is marked as such below rather than given a
...
consecutive beads in the smooth stage, the bond between consecutive block centroids in block
placement, the bond a consecutive arcless anchor pair gets in the arcs stage when
`use_arcs_chain_bonds` is on, and the background of the unified arc target. It is calibrated for
consecutive beads a kb or so apart. Its exponent under the production values is 0.75, and the
distance it gives grows far faster with separation than the polymer physics predicts. Contact
probability falls close to the inverse of separation in Hi-C [4], and if distance grows as
separation to the power `nu`, contact probability falls as separation to the power `-3 nu` [5],
so a contact slope near `-0.86`, which is what this project's three cell lines measure, means
distance should grow as separation to about the power 0.285. That is the target quoted
throughout, and it matters wherever the chain law is evaluated at large separations.

**The PET law**, `[distance] count_dist_*`, gives an arc's target from its PET count alone,
`base_level + scale / exp(a * (PET + shift))` [1]. It runs from its zero PET value down to
...
are set and before the chain bonds are added, so contact between neighbours does not shrink a
bond.
...
[1], `max(0, 1 / d - 1 / (arcs_repulsion_cutoff_factor * mean arc target))`, unbounded when the
...
fixed. So an anchor's position is set in the arcs stage and nothing after it moves an anchor.
What the arcs stage matrix asks for is what the structure gets.
