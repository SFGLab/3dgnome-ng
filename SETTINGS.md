...
| `arc_weight_exponent` | float | 0.0 | 0.0 | Each loop's spring constant scaled by its strength, the PET count over the typical count at its span, to this power; zero is every loop at one spring, byte exact. Under measurement, design/expression-from-structure.md idea 37. |
